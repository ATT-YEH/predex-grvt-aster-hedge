...
Supported exchanges:
    ...
    - grvtaster: Uses HedgeBot from hedge_mode_grvtaster.py (GRVT + Aster)
...
Examples:
    ...
    python hedge_mode.py --exchange grvtaster --ticker ETH --size 0.05 --iter 20
        """
    )

    parser.add_argument('--exchange', type=str, required=True,
                        help='Exchange combo to use (e.g., backpack, extended, grvtaster)')
...
def validate_exchange(exchange):
    ...
    # 🌟 修正點 1: 新增 grvtaster 支援
    supported_exchanges = ['backpack', 'extended', 'grvtaster']
...
def get_hedge_bot_class(exchange):
    ...
    try:
        if exchange.lower() == 'backpack':
            ...
            return HedgeBot
        # 🌟 修正點 2: 新增 grvtaster 導入邏輯
        elif exchange.lower() == 'grvtaster':
            from hedge.hedge_mode_grvtaster import HedgeBot
            return HedgeBot
        else:
            # 應該永遠不會到達這裡，因為上面已經驗證過
            raise ValueError(f"Unsupported exchange: {exchange}")
    except ImportError as e:
        print(f"Error importing hedge mode implementation for {exchange}: {e}")
        sys.exit(1)
...
async def main():
    ...
    try:
        # Create the hedge bot instance (參數傳遞給 HedgeBotGA/HedgeBot)
        # 由於所有 HedgeBot 的 __init__ 簽名一致，我們可以簡化實例化
        bot = HedgeBotClass(
